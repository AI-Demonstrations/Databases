...
# The Gremlin client needs just the raw PRIMARY KEY, not a full connection
# string. Accept either -- if a full "AccountEndpoint=...;AccountKey=...;"
# string was pasted, pull the key back out of it.
cosmoskey = os.getenv("COSMOS_CONNECTION_STRING_GRAPH", "")
if "AccountKey=" in cosmoskey:
    for part in cosmoskey.split(";"):
        part = part.strip()
        if part.startswith("AccountKey="):
            cosmoskey = part[len("AccountKey="):]
            break

# Accepts either a bare hostname (e.g. "your-account.documents.azure.com")
# or a full URL as shown in the Azure Portal (e.g. "wss://your-account...:443/")
gremlin_account_host = os.getenv("COSMOS_GREMLIN_ENDPOINT", "")
for prefix in ("https://", "wss://", "http://", "ws://"):
    if gremlin_account_host.startswith(prefix):
        gremlin_account_host = gremlin_account_host[len(prefix):]
gremlin_account_host = gremlin_account_host.split(":")[0].split("/")[0]

gremlin_database = os.getenv("COSMOS_GREMLIN_DATABASE", "demograph")
gremlin_graph = os.getenv("COSMOS_GREMLIN_GRAPH", "playgraph")
...
_gremlin_insert_vertices = [
    "g.addV('person').property('demo2','hello').property('id', 'thomas').property('firstName', 'Thomas').property('age', 44).property('testdb', 'demo2')",
    "g.addV('person').property('demo2','hello2').property('id', 'mary').property('firstName', 'Mary').property('lastName', 'Andersen').property('age', 39).property('testdb', 'demo2')",
    "g.addV('person').property('demo2','hello3').property('id', 'ben').property('firstName', 'Ben').property('lastName', 'Miller').property('testdb', 'demo2')",
    "g.addV('person').property('demo2','hello4').property('id', 'robin').property('firstName', 'Robin').property('lastName', 'Wakefield').property('testdb', 'demo2')"
]
...
try:
    ...
    hostname = "wss://{0}:443/".format(gremlin_account_host)

    client = client.Client(hostname, 'g',
                           username="/dbs/{0}/colls/{1}".format(gremlin_database, gremlin_graph),
